  ...
  try {
    ...
    if (error) {
      res.status(400).json({
        success: false,
        message:
          error.details?.[0]?.message || MESSAGES.VALIDATION.FAILED,
      });
      return;
    }
    if (req.file) {
...
export const getAllProductsHandler = async (
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : MESSAGES.PRODUCT.GET_FAILED;
...
export const getProductByIdHandler = async (
  ...
  try {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      res
        .status(400)
        .json({ success: false, message: MESSAGES.VALIDATION.FAILED });
      return;
    }
...
export const deleteProductHandler = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: MESSAGES.PRODUCT.DELETE_SUCCESS });
  } catch (error: unknown) {
    const message =
